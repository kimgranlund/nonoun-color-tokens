...
// This generator proves the SAME anchor-splice technique at BUILD time, for two pieces that used to be
...
// gen:figma-assets, which embeds this file's post-splice content into the download). The `parity` and
// `floatparity` gates in test/figma/binder.mjs are now a TRIPWIRE over this generator's output — they
// prove the splice actually matches the canonical source, not the mechanism preventing drift.
...
// 2) roleTable(paletteName) — semanticRoles()'s body, re-wrapped; plus its 3 supporting SCRIM_* consts,
...
console.log("wrote figma/binder/figma-semantic-binder/code.js (spliced FLOAT_EXECUTOR + ROLE_TABLE from canonical sources)");
