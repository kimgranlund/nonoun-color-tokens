  ...
  if (inc.type && scale && scale.categories && typeof scale.categories === "object") {
    for (const [voice, steps] of Object.entries(scale.categories)) {
      ...
      for (const step of ["LG", "MD", "SM"]) {
        ...
        // collide with a sibling's own name. The label itself prefers the voice's own custom Figma
        // style name (`coreStyleName` — e.g. BZZR's "Condensed Black Italic") over the generic
        // ladder-snap name ("Black") — a non-variable face's real cut is strictly more specific and
        // must never be flattened down to the ladder's generic vocabulary.
        const coreWeightName = sibs.length ? weightNameFor(s.weight) : null;
        const coreLabel = coreWeightName ? (coreStyleName || coreWeightName.name) : null;
        texts.push({
