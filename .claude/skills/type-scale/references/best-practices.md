...
- **No magic numbers in a treatment.** The whole point of the engine (mirroring color) is that every step's
  ...
  The type.mjs header says it: "no hand-authored magic numbers".
- **Resize with `bodyBase`, not individual bases.** `factor = bodyBase / Body.base` scales every voice
  ...
  bodyBase-scaling assert (`test/engine/type.mjs`) asserts a larger bodyBase scales Display up too — that
  lockstep is the feature.
...
- **Always emit family names QUOTED** (`--font-{role}: '{family}'` in `typeTokensCSS`). A name with a digit —
  ...
  Serif 4'` (the luxury quoting assert). If you refactor the emitter, keep the quotes. This is the type analog of color's
