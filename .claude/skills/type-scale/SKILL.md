...
description: >
  ...
  font swap, a fallback-font render, or a red type gate. Covers the thirteen named voices (Display ·
  Headline · Sub-heading · Title · Sub-title · Lead · Body · Body-mono · Label · Label-mono · Kicker ·
  Tiny · Tiny-mono), the five treatments, and the self-hosted fonts. TYPE sibling of
...
table shared across all treatments (see the layer table below). The conceptual *why* (the thirteen voices,
...
| **`makeVoices(o={})`** | the FACTORY — returns the THIRTEEN named voices, sharing structure, reading per-voice knobs from `o` | `Display · Headline · Sub-heading · Title · Sub-title · Lead · Body · Body-mono · Label · Label-mono · Kicker · Tiny · Tiny-mono` |
...
- **Voice taxonomy** — the thirteen voices × the single uniform 3-step ramp (SM/MD/LG; the old per-voice `STEPS_3`/`STEPS_5`/`STEPS_UI` split is retired), the `roleOf` mapping (Body-mono/Label-mono/Kicker/Sub-title/Tiny-mono → `mono`; Lead/Body → `body`; Label/Tiny → `ui`), the `box` flag that decouples the presentation FLOW from the font role (the actual BOX voices are exactly Label/Body-mono/Label-mono/Kicker — Sub-title/Tiny/Tiny-mono ride a box-default role but are `box:false` PROSE), the caps voices, the per-treatment case rules → foundations §2 + §4.
...
4. **New treatment? — add the THIRTEEN voices by passing the full `fonts` palette** (`display/heading/body/ui/mono` — five roles) so `roleOf` resolves every voice, and supply `note` (the UI specimen copy reads it). The test asserts every treatment has all thirteen voices + `fonts`.
...
```
node test/engine/type.mjs    # 5 treatments × 13 voices · roleOf (Body-mono/Label-mono/Kicker/Sub-title/
...
| `references/rubric.md` | score the change before calling it done — the thirteen voices, the fixed-size-table math, the quoting guard, the body-class snap boundary, and the font wiring are the gates |
...
| `docs/reference/typography/README.md` | the thirteen named voices, the fixed-size-table rewrite, the system relationships, the target token shape — owned there, cite |
