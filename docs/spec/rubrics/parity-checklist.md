...
| Impl | File | Carries |
|------|------|---------|
| Artifact (the tool) | `hct-palette-generator.html` (inline `<script>`) | engine + `semanticRoles` + exporters + UI |
...
## Checks
- **P1 — Role count.** `semanticRoles('primary').length === 37` in all three and in
