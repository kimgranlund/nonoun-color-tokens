...
description: >
  ...
  clamped to the sRGB gamut ceiling by construction) plus a 59-role semantic token layer,
...
(that stays CAM16/L\*, ADR-001); no plugin-free aliased export in this revision (resolved colors only, ADR-002;
...
```json
{
  ...
  "acceptance_criteria": [
    ...
    { "id": "hpg-semantic-roles", "check": "semanticRoles(n) returns exactly 59 roles for every palette, of which exactly 7 are scrims on the 500 translucency ramp (scrimWeakest..scrimStrongest, each = the palette's 500 stop at alpha% = step/10, light === dark; outline + container/Low/High also resolve onto this 500 ramp) (AC-S1, AC-S2)" },
    ...
    { "id": "hpg-export-leaf-valid", "check": "each of Light_tokens.json and Dark_tokens.json contains at least 59 x (enabled palettes) resolved color leaves, each with $type 'color', colorSpace 'srgb', components in [0,1], alpha in [0,1], and a hex matching components (hex channel == Math.round(component*255) as 2-digit uppercase) (AC-X5) — an empty semantic tree FAILS (non-vacuity floor)" },
...
4. **One source of truth.** Build the engine + the 59-role table ONCE (a module set imported
...
| `data/role-table.json` | canonical 59-role table, defaults, constants (machine-readable) |
