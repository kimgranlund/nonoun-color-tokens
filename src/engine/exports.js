...
export function exportDesignSystemReceipt(state, typeSc, geomSc, opts = {}) {
  const ds = dsColorRoles(state);
  const profile = opts.profile === "google-stitch" ? "google-stitch" : "claude-code";
  ...
  const divLines = div.map((t) => `- ℹ️ DIVERGENCE (authorial, called out per the standing rule): \`${t.name}\` = \`${t.light.hex}\` in both schemes — a light fill that takes a near-black label in both schemes (rationale in DESIGN.md Colors); the §8 carrier gate flags it on every run and it is disclosed here.`);
  ...
  const contrastLines = [
    "- 🟢 Contrast: all declared fill/on-pairs ≥ 4.5:1, both schemes, all-pairs policy (F1 fixed — dark-scheme",
