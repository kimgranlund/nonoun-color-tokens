...
// dsStateFills — the MEASURED hover/active fills for one family in one scheme (§7 R3). The grammar
// requires ONE `{family}-on-{family}` that clears 4.5:1 on the base AND every emitted state fill —
// but the role table's mode-mirrored state direction (hover darkens in light / lightens in dark)
// assumes the scheme's fill-text pole. A bright brand fill that takes the INK label in light (the
// measured pole) breaks that assumption: the table's darker hover moves AGAINST the ink label
// (a real AA failure the default theme masked by pole-direction luck). So: keep the table's state
// ref when it still clears 4.5 with the CHOSEN on-color; otherwise step from the measured base fill
// in the label's contrast direction (ink label → lighter stops; paper label → darker), hover one
// passing step out, active one further. At a ramp end the state holds the last passing stop (a
// no-op state beats a failing one).
function dsStateFills(p, scheme, fillRgb, onRgb) {
  const stops = Object.keys(p.stops).map(Number).sort((a, b) => a - b);
  const at = (s) => p.byStop.get(s);
  const pass = (rgb) => rgb && dsContrast(onRgb, rgb) >= DS_AA;
  const tableEnd = (suffix) => { const r = dsRole(p, suffix); return r ? r[scheme].rgb : null; };
  const tHover = tableEnd("-hover"), tActive = tableEnd("-active");
  if (pass(tHover) && pass(tActive)) return { hoverRgb: tHover, activeRgb: tActive }; // table holds — keep it
  // find the measured base fill on the ramp (dsFillOn may have stepped it); fall back to the prime ref.
  let idx = stops.findIndex((s) => { const c = at(s); return c && rgbEq3(c, fillRgb); });
  if (idx < 0) { const prime = dsRole(p, ""); idx = stops.indexOf(Number(scheme === "light" ? prime.lightRef : prime.darkRef)); }
  // label's contrast direction: a dark (ink) label wants LIGHTER fills (lower stops, dir −1),
  // a light (paper) label darker fills (higher stops, dir +1).
  const dir = dsContrast([0, 0, 0], onRgb) < dsContrast([255, 255, 255], onRgb) ? -1 : +1;
  const stepTo = (from) => { for (let j = from + dir; j >= 0 && j < stops.length; j += dir) if (pass(at(stops[j]))) return j; return from; };
  const h = stepTo(idx), a = stepTo(h);
  return { hoverRgb: at(stops[h]) || fillRgb, activeRgb: at(stops[a]) || fillRgb };
}
const rgbEq3 = (a, b) => { const ar = a.rgb || a, br = b.rgb || b; return ar[0] === br[0] && ar[1] === br[1] && ar[2] === br[2]; };
...
export function dsColorRoles(state) {
  ...
  // hover/active are MEASURED against the chosen on-color (dsStateFills) — the table ref where it
  // holds, a step in the label's contrast direction where it doesn't.
  const chSL = dsStateFills(chrome, "light", chL.fillRgb, chL.onRgb), chSD = dsStateFills(chrome, "dark", chD.fillRgb, chD.onRgb);
  push(`${cn}-hover`, rgbEnd(chSL.hoverRgb), rgbEnd(chSD.hoverRgb));
  push(`${cn}-active`, rgbEnd(chSL.activeRgb), rgbEnd(chSD.activeRgb));
  ...
  for (const p of [...others, ...intents]) {
    ...
    // states MEASURED against the chosen on-color (never the raw table ref when it would fail AA —
    // a bright fill takes the ink label, and the table's darker light-mode hover moves against it).
    const sL = dsStateFills(p, "light", fl.fillRgb, fl.onRgb), sD = dsStateFills(p, "dark", fd.fillRgb, fd.onRgb);
    push(`${p.n}-hover`, rgbEnd(sL.hoverRgb), rgbEnd(sD.hoverRgb));
    if (p === brandPal) push(`${p.n}-active`, rgbEnd(sL.activeRgb), rgbEnd(sD.activeRgb));
  ...
  return { chrome, poles, tokens, alias, aliasDistinct, families };
}
...
function dsSpineBody(ds, state, ctx) {
  ...
  const components = [
    ...
    "  carries its own `-hover` and `-disabled` — any intent is a real button; every state fill is measured ≥4.5:1 with the family's `on-{family}`, stepping in the label's contrast direction.",
...
export function exportDesignSystemReceipt(state, typeSc, geomSc, opts = {}) {
  ...
  if (profile === "figma-make") {
    return [
      ...
      "  canonical core, NOT the config's fixed white which fails AA on the brightened dark fills); each",
...
export function exportDesignSystemMakeBundle(state, typeSc, geomSc, opts = {}) {
  ...
  // One contrast-mode state feeds BOTH the projection and the appended full layer, so every shadcn
  // token can be a var() LINK into the design-token layer and resolve to the identical measured value.
  // aliasPrefix wires the links; radii/fonts seed --radius and the @theme font slots from the real
  // scales (previously omitted — the radius silently fell back to shadcn's 0.625rem default and the
  // font vars setup.md promises were absent).
  const contrastState = { ...state, onColorMode: "contrast" };
  const shadcnCss = exportShadcn(contrastState, { aliasPrefix: pfx, radii: dsRadii(geomSc), fonts: (typeSc && typeSc.fonts) || {} });
  const stylesCss = shadcnCss + dsFullLayersCss(contrastState, typeSc, geomSc);
  return [
