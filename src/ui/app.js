...
class HctApp extends HTMLElement {
      ...
      h(
        ...
        ...Object.entries(scale.fonts).map(([role, family]) => {
          ...
          return h(
            ...
            (() => { const st = this._fontStatus(family); return h("span", { class: "tyi-font-badge is-" + st.state, title: st.title, "data-fk": "tyfontbadge:" + role }, st.label); })(),
          );
        }),
      ),
  ...
  _zipReadme(s, sys) {
    ...
    return [
      ...
      "```", "/plugin marketplace add kimgranlund/ultimate-tokens", "/plugin install ultimate-tokens", "```", "",
