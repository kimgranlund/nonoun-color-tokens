...
class HctApp extends HTMLElement {
  ...
  // canvasBg — the canvas backdrop = the SELECTED palette's EDGE color: its light end (the 050 stop)
  // in light preview, its dark end (the 950 stop) in dark preview. So the backdrop carries that
  // palette's own hue at the extreme (vs. the old neutral gray), and follows BOTH palette selection
  // (selectPalette → render) and lmin/lmax (which set the edge tones). At lmax=100 the 050 stop is
  // pure white by the white-endpoint guarantee, so the light backdrop reads white until lmax is lowered.
  // Falls back to a neutral gray at the tonal extreme if no palette/ramp is available yet.
  canvasBg() {
    const v = this._view || projectView(this.doc);
    const pal = v && v.palettes[this.selectedIndex()];
    const stop = pal && pal.ramp.find((s) => s.stop === (this.canvasTheme === "dark" ? 950 : 50));
