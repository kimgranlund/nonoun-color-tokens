...
class HctApp extends HTMLElement {
  ...
  // canvasBg — the canvas backdrop neutral, driven by the GLOBAL tonal range:
  // light preview sits at L*=lmax (the light end of every ramp), dark preview at
  // L*=lmin (the dark end), so the backdrop tracks the extremes the ramps are
  // generated to. chroma 0 -> the engine's gray branch -> a [v,v,v] neutral.
  canvasBg() {
    const L = this.canvasTheme === "dark"
      ? (this.doc.lmin ?? 5)
      : (this.doc.lmax ?? 100);
    const v = hctToRgb(0, 0, L).rgb[0];
    ...
    return h(
      ...
      h(
        ...
        h(
          ...
          // ONE download action — every format in its own folder + the config, as a single .zip. Always present, primary.
          h("button", { class: "primary", title: "Download every format (css-hex, css-oklch, json, dtcg, figma, ui3) in its own folder + the config, as one .zip", onclick: () => this.downloadAllZip(view) }, "⬇ Download all (.zip)"),
          // (Config save/load lives in the config sub-bar above the code; not a footer action.)
          // Inside Figma, applying variables directly is the point — so it's the primary action.
          this.inFigma
            ? h(
                "button",
                {
